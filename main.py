...
button1 = tk.Button(root, text="Convert Egg", command=lambda : threading.Thread(target=change_egg).start())
button1.pack()
...
label1.pack()
button2 = tk.Button(root, text='DECIDE', command=lambda : threading.Thread(target=make_decision).start())
button2.pack()
